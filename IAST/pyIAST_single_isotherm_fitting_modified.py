...
class ModelIsotherm:
    ...
    * Henry's law. Only use if your data is linear, and do not necessarily trust
    ...
    def __init__(self,
        ...
        self._fit(optimization_method,param_bnds)
    ...
    def _fit(self, optimization_method, param_bnds):
        """
        ...
        def residual_sum_of_squares_log(params_):
            """
            Residual Sum of Squares between the transformed values of model and data in df
            How data is transformed: if loading>0.1, keep the original value, else take the log10
            To better implement this, a 1e-10 is added to all values<0.1 to avoid 0s
            This is to generate good fitting results in low pressure regime
            :param params_: Array of parameters
            """
            # change params to those in x
            for i in range(len(param_names)):
            ...
            def loading_transform(loading_to_be_transformed):
                return np.where(loading_to_be_transformed>0.1, loading_to_be_transformed, np.log10(abs(loading_to_be_transformed)+1e-10))


            return np.sum((loading_transform(self.df[self.loading_key].values) - loading_transform(self.loading(
                self.df[self.pressure_key].values)))**2)

        # minimize RSS
        opt_res = scipy.optimize.minimize(
            residual_sum_of_squares_log, guess, method=optimization_method,bounds=param_bnds)
    ...
    def spreading_pressure(self, pressure):
        ...
        if self.model == "Quadratic":
            return self.params["M"] * np.log(1.0 + self.params["Ka"] * pressure + self.params["Kb"] * pressure**2)

        if self.model == "BET":
            return self.params["M"] * np.log((1.0 - self.params["Kb"] * pressure + self.params["Ka"] *pressure) / (1.0 - self.params["Kb"] * pressure))

        if self.model == "DSLangmuir":
            return self.params["M1"] * np.log(1.0 + self.params["K1"] * pressure) +\
                   self.params["M2"] * np.log(1.0 + self.params["K2"] * pressure)
        ...
        if self.model == "TemkinApprox":
            ...
            return self.params["M"] * (np.log(one_plus_kp) + self.params["theta"] *(2.0 * self.params["K"] * pressure + 1.0) /(2.0 * one_plus_kp**2))
...
class InterpolatorIsotherm:
    ...
    def spreading_pressure(self, pressure):
        ...
        if n_points == 0:
            ...
            slope = (self.loading(pressure) - loadings[n_points - 1]) / (pressure - pressures[n_points - 1])
